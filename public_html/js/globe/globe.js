 ...
 DAT.Globe = function(container, opts) {
     ...
     'earth' : {
       ...
       fragmentShader: [
        ...
        //    'vec3 diffuse = texture2D( texture, vUv ).xyz;',
           'vec3 diffuse = vec3( 0.02, 0.02, 0.08 );',
           'float intensity = 1.1 - dot( vNormal, vec3( 0.13, -0.13, 0.9 ) );',
           'vec3 atmosphere = vec3( 0.7, 0.8, 1.0 ) * pow( intensity, 3.0 );',
   ...
   function init() {
    ...
    function trajectoryFromTo(startLat, startLng, endLat, endLng, settings) {
        ...
        const resolution = settings.resolution || 30;
        const color = settings.color || 0xff0000
        const material = settings.material || new THREE.LineDashedMaterial( {color, dashSize: 0, gapSize: 100000, scale: 1} );
        ...
        //         effects.splice(effects.indexOf(effects), 1);
        //     }
        // }
        return {
   ...
   function initSpline() {
        ...
        * testing
        */
        let curve = getSplineFromCoords([ 50.800209, 6.764670, 0, 0 ])
        ...
        const homePoint = coordinateToPosition(50.800209, 6.764670, GLOBE_RADIUS);

        geometry = new THREE.BoxGeometry( 50, 50, 50);
        geometry.position = homePoint;

        material = new THREE.MeshBasicMaterial( {color: 0x00ff00} );
        const cube = new THREE.Mesh( geometry, material );
        cube.position.set(homePoint.x, homePoint.y, homePoint.z);
        scene.add( cube );

        let lat = 0;
        window.setInterval(() => {
            const newPos = coordinateToPosition(lat, 0, GLOBE_RADIUS);
